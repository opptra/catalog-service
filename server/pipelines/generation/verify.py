...
def _verify_suffix(
    *,
    ...
    return (
        ...
        "and label from this image. Split them: on-product print (woven, stamped, or "
        ...
        "On-product lettering on a source photo is identity, not a claim.\n"
        f"{photo_line}\n\n"
        f"{slot_block}\n\n"
        "Score THREE axes (integers 0–100):\n"
        "- identity: does the GENERATED IMAGE show the same physical variant as the "
        "source photos and PRODUCT CARD identity (hang/drop, opening, Color / pack / "
        "print / silhouette), including letters that are physically on the product. "
        "A window-height product rendered floor-length (or the reverse) is an identity "
        "miss even if colour and print match. If source photos show on-product "
