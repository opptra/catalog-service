...
def _fact_board_prompt(product: dict[str, Any], claims: list[str]) -> str:
    ...
    return (
        ...
        "- Return items only for claims that add a shopper fact not already returned. "
        "If a claim is only a restatement of a fact already covered, return nothing "
        "for it.\n"
        "- A name, style, or title cell that only concatenates specs already returned "
        "(material, print, colour, count) is a restatement — omit it. Prefer the "
        "atomic structured fields. That bundle is not an extra independent spec.\n"
