...
_RULES = (
    ...
    "- For ANY image (hero, infographic, lifestyle, enhanced brand modules, or any other type): "
    "do NOT draw, render, watermark, or place any brand logo or brand name on the image. Do NOT "
    "leave empty reserved space, corners, banners, margins, or padding for a logo — the logo is "
    "added later by a deterministic code step, so compose the full frame with product/content "
    "only.\n"
    "- On-image text must be shopper-facing only. Never write internal or ops labels on the "
    "image — including A+, A+ Content, A+ Features, Enhanced Brand Content, EBC, PDP, gallery "
    "slot names, attribute type codes (IMAGE, A_PLUS), or similar backend/module jargon. Show "
    "features, benefits, and care as normal product copy, not as labelled 'A+' modules."
)
...
def gallery_plan_prompt(ctx: GenerationContext, name: AttributeName, quantity: int) -> str:
    """Plan ONE coherent, non-duplicated image set for a single attribute type via a tool call.

    Count comes from the UI request. When Category Intelligence includes ``image_plan``, treat it
    as a recommended role palette (core first) — not a fixed 1:1 slot assignment. Translate those
    ideas into distinct render prompts for this product. Aspect ratio and logo placement stay out
    of the model's job (fixed per attribute type in code; logo composited downstream).

    Full Brand DNA is NOT included — use ``ctx.common_image_context`` (distilled brand + category
    visual bits) so every plan shares the same typography/palette/norms.
    """
    brief = category.image_brief(ctx.category_intelligence, [name])
    # Internal type codes (IMAGE / A_PLUS) are for tooling only — never shopper-facing copy.
    type_label = (
    ...
    return (
        ...
        "typefaces.\n\n"
        "SHOPPER-FACING COPY (mandatory):\n"
        "- Every on-image headline, badge, and body line must read as something a shopper should "
        "see on the live listing.\n"
        "- Never put internal or ops labels on the image: do not write A+, A+ Content, A+ "
        "Features, A+ Care, Enhanced Brand Content, EBC, PDP, IMAGE, A_PLUS, slot numbers, or "
        "similar jargon. Feature and care modules must use normal titles (e.g. Features, Care "
        "instructions) — never brand them as A+.\n"
        "- Attribute type names and image_plan role codes are planning hints only; translate "
        "them into real product messaging, do not print them on the artwork.\n"
        "- On-image text (when used): phone-readable, minimal, SEO-useful keywords from product/"
        "category — no fluff walls of text.\n\n"
...
def revise_generation_prompt(
    *,
    ...
    return (
        ...
        + (
            "On-image text must stay shopper-facing: never include A+, A+ Content, A+ Features, "
            "EBC, PDP, IMAGE, A_PLUS, or other internal/ops labels in the revised prompt's "
            "on-image copy instructions.\n"
        ...
        + "When finished, call the submit_revised_prompt tool with the final prompt string — "
        "do not write the prompt as free-form JSON in the message body.\n\n"
        f"PREVIOUS PROMPT:\n{previous_prompt}\n\n"
        f"{current_block}\n\n"
        f"USER IMPROVEMENT:\n{improvement.strip()}"
    )
